...
public abstract class BTreeNode<TKey extends Comparable<TKey>> {
  ...
  public BTreeNode<TKey> dealOverflow() {
    ...
    if (this.getRightSibling() != null) this.getRightSibling()
      .setLeftSibling(newRNode);
  ...
  protected abstract BTreeNode<TKey> pushUpKey(
    TKey key,
  ...
  public BTreeNode<TKey> getLeftSibling() {
    if (
      this.leftSibling != null &&
      this.leftSibling.getParent() == this.getParent()
  ...
  protected abstract void processChildrenTransfer(
    BTreeNode<TKey> borrower,
    BTreeNode<TKey> lender,
    int borrowIndex
  );

  protected abstract BTreeNode<TKey> processChildrenFusion(
    BTreeNode<TKey> leftChild,
    BTreeNode<TKey> rightChild
  );

  protected abstract void fusionWithSibling(
    TKey sinkKey,
    BTreeNode<TKey> rightSibling
  );

  protected abstract TKey transferFromSibling(
    TKey sinkKey,
