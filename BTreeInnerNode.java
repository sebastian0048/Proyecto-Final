public class BTreeInnerNode<TKey extends Comparable<TKey>>
  extends BTreeNode<TKey> {

  protected static final int INNERORDER = 4;
  ...
  @Override
  protected BTreeNode<TKey> pushUpKey(
    TKey key,
    BTreeNode<TKey> leftChild,
  ...
  @Override
  protected void processChildrenTransfer(
    BTreeNode<TKey> borrower,
    BTreeNode<TKey> lender,
    int borrowIndex
  ) {
    int borrowerChildIndex = 0;
    while (
      borrowerChildIndex < this.getKeyCount() + 1 &&
    ...
    if (borrowIndex == 0) {
      ...
      TKey upKey = borrower.transferFromSibling(
        this.getKey(borrowerChildIndex),
        lender,
        borrowIndex
      );
      ...
      TKey upKey = borrower.transferFromSibling(
        this.getKey(borrowerChildIndex - 1),
        lender,
        borrowIndex
      );
  ...
  @Override
  protected BTreeNode<TKey> processChildrenFusion(
    BTreeNode<TKey> leftChild,
    BTreeNode<TKey> rightChild
  ) {
    int index = 0;
    while (
      index < this.getKeyCount() && this.getChild(index) != leftChild
    ) ++index;
  ...
  protected void fusionWithSibling(TKey sinkKey, BTreeNode<TKey> rightSibling) {
    ...
    if (
      rightSiblingNode.rightSibling != null
    ) rightSiblingNode.rightSibling.setLeftSibling(this);
  }

  @Override
  protected TKey transferFromSibling(
    TKey sinkKey,
    BTreeNode<TKey> sibling,
    int borrowIndex
  ) {
    ...
    if (borrowIndex == 0) {
      ...
      this.insertAt(
          0,
          sinkKey,
          siblingNode.getChild(borrowIndex + 1),
          this.getChild(0)
        );
